...
int main(int argc, char** argv)
{
	...
	char* source = (char*)calloc(sizeof(char) * USIZE * USIZE * USIZE, sizeof(fp));
	...
	int max_work_group_size_x = cbrt(max_work_group_size);
	int max_work_group_size_y = cbrt(max_work_group_size);
	int max_work_group_size_z = cbrt(max_work_group_size);
	int batch_size_x = (USIZE - 2) / max_work_group_size_x;
	int batch_size_y = (USIZE - 2) / max_work_group_size_y;
	int batch_size_z = (USIZE - 2) / max_work_group_size_z;
	printf("# %d\n# %d\n# %d\n", batch_size_x, batch_size_y, batch_size_z);
	...
	cl_mem sourceb = clCreateBuffer(context, CL_MEM_READ_ONLY
			| CL_MEM_USE_HOST_PTR, n * sizeof(char), source,
			&errNum);
	verbose(errNum, "Error creating geometryb");
	...
	errNum = clEnqueueWriteBuffer(queue, sourceb, CL_TRUE, 0, USIZE * USIZE * USIZE* sizeof(char),
			source, 0, NULL, NULL);
	verbose(errNum, "Error writing sourceb buffer");
	...
	errNum = clSetKernelArg(kernel, 20, sizeof(fp), (void*)&max_time);
	verbose(errNum, "Error setting max_time");
	errNum = clSetKernelArg(kernel, 21, sizeof(fp), (void*)&time_step);
	verbose(errNum, "Error setting time_step");
	errNum = clSetKernelArg(kernel, 22, sizeof(int), (void*)&batch_size_x);
	verbose(errNum, "Error setting batch_size_x");
	errNum = clSetKernelArg(kernel, 23, sizeof(int), (void*)&batch_size_y);
	verbose(errNum, "Error setting batch_size_y");
	errNum = clSetKernelArg(kernel, 24, sizeof(int), (void*)&batch_size_z);
	verbose(errNum, "Error setting batch_size_z");
	errNum = clSetKernelArg(kernel, 25, sizeof(cl_mem), (void*)&sourceb);
	verbose(errNum, "Error setting sourceb");
	errNum = clSetKernelArg(kernel, 26, sizeof(fp), (void*)&ramped_sin_amplitude);
	verbose(errNum, "Error setting source_amplitude");
	errNum = clSetKernelArg(kernel, 27, sizeof(fp), (void*)&ramped_sin_length);
	verbose(errNum, "Error setting sourcce ramp length");
	errNum = clSetKernelArg(kernel, 28, sizeof(fp), (void*)&ramped_sin_frequency);
	verbose(errNum, "Error setting source_amplitude");



	// Invoke the kernel
	size_t gws[3] = {(USIZE - 2) / batch_size_x, (USIZE - 2) / batch_size_y, (USIZE - 2) / batch_size_z};
	errNum = clEnqueueNDRangeKernel(queue, kernel, 3, NULL, gws, gws, 0, NULL, NULL);
	...
	for (int z = 0; z < USIZE; z++)
	{
		for (int y = USIZE / 2; y < USIZE / 2 + 1; y++)
		{
			for (int x = 0; x < USIZE; x++)
			{
				int id = x + y * USIZE + z * USIZE * USIZE;
