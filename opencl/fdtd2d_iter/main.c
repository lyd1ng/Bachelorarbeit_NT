...
int main()
{
	...
	ramped_sin_length = 10 * time_step;

	// Device characteristics
	int max_work_group_size = 256;
	int max_work_group_size_x = sqrt(max_work_group_size);
	int max_work_group_size_y = sqrt(max_work_group_size);
	int batch_size_x = (UWIDTH - 2) / max_work_group_size_x;
	int batch_size_y = (UHEIGHT - 2) / max_work_group_size_y;
	...
	cl_mem old_ez_universeb = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
	...
	cl_mem old_hy_universeb = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
	...
	errNum = clEnqueueWriteBuffer(queue, sourceb, CL_TRUE, 0, UWIDTH * UHEIGHT* sizeof(char),
			source, 0, NULL, NULL);
	...
	errNum = clSetKernelArg(kernel, 22, sizeof(fp), (void*)&max_time);
	verbose(errNum, "Error setting max_time");
	errNum = clSetKernelArg(kernel, 23, sizeof(fp), (void*)&time_step);
	verbose(errNum, "Error setting time_step");
	errNum = clSetKernelArg(kernel, 24, sizeof(int), (void*)&batch_size_x);
	verbose(errNum, "Error setting batch_size_x");
	errNum = clSetKernelArg(kernel, 25, sizeof(int), (void*)&batch_size_y);
	verbose(errNum, "Error setting batch_size_y");
	errNum = clSetKernelArg(kernel, 26, sizeof(cl_mem), (void*)&sourceb);
	verbose(errNum, "Error setting sourceb arg");
	errNum = clSetKernelArg(kernel, 27, sizeof(fp), (void*)&ramped_sin_amplitude);
	verbose(errNum, "Error setting ramped_sin_amplitude");
	errNum = clSetKernelArg(kernel, 28, sizeof(fp), (void*)&ramped_sin_length);
	verbose(errNum, "Error setting ramped_sin_length");
	errNum = clSetKernelArg(kernel, 29, sizeof(fp), (void*)&ramped_sin_frequency);
	verbose(errNum, "Error setting ramped_sin_frequency");


	// Invoke the kernel
	size_t gws[2] = {(UWIDTH - 2) / batch_size_x, (UHEIGHT - 2) / batch_size_y};
	errNum = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, gws, gws, 0, 0, 0);
	...
	verbose(errNum, "Error reading the kernel");
